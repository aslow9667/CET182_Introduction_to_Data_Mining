...
# ============================ Clustering =====================================                        # To comment out Classification section when running Clustering
...
'''Plotting the cluster member's attributes'''
...
#======================== Classification =====================================                        # To comment out Clustering section when running Classification
''''Data Pre-Processing: One Hot Encoding ''' 
pred_categoricalCols = ['workclass', 'marital-status','occupation',\
                   'relationship','race','sex','native-country']   

onehotencoder = OneHotEncoder()

for col in pred_categoricalCols:
    X = onehotencoder.fit_transform(loadedData_noMissingValue[col].values.reshape(-1,1)).toarray()
    dfOneHot = pd.DataFrame(X, columns = onehotencoder.get_feature_names([col])) 
    loadedData_noMissingValue = pd.concat([loadedData_noMissingValue, dfOneHot], axis=1)
    loadedData_noMissingValue = loadedData_noMissingValue.drop([col], axis=1) 
    print ('Completed OneHotEncoded for the Categorical Column: ' + col)

labelencoder = LabelEncoder()
loadedData_noMissingValue['gross_income'] = labelencoder.fit_transform(loadedData_noMissingValue['gross_income'])

''' Data Preprocesssing - scaling/ splitting ''' 
clf = RandomForestClassifier()
scaler = StandardScaler()
y = loadedData_noMissingValue.pop('gross_income')
feature_name = list(loadedData_noMissingValue.columns)
x = scaler.fit_transform(loadedData_noMissingValue)
X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.3, random_state=42)

''' Grid Search - Hyperparameter'''
param_grid = {'n_estimators': [10, 20, 30, 40],'max_depth' : [4, 8, 12, 16]}
